...
def apply_mod(tensor, m_mult, m_add=None, modulation_dims=None):
    if modulation_dims is None:  # Case where modulation dimensions are not provided
        if m_add is not None:
            # Use numpy's broadcasting and vectorized operations for efficiency
        ...
        if m_add is not None:
            for d in modulation_dims:
                # Combine multiplication and addition in a single step
                tensor[:, d[0] : d[1]] = (
                    tensor[:, d[0] : d[1]] * m_mult[:, d[2]] + m_add[:, d[2]]
                )
        else:
            for d in modulation_dims:
