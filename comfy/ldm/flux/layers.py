...
class ModulationOut:
    ...
    def __init__(self, *args):
        # Assumes that ModulationOut is a simple container for outputs.
        self.args = args
...
class Modulation(nn.Module):
    ...
    def forward(self, vec: Tensor) -> tuple:
        # Optimize vec.ndim adjustment by avoiding unnecessary reshape if vec already has 3 dimensions.
        if vec.ndim == 2:
            vec = vec.unsqueeze(1)
        # Use silu directly from torch.nn.functional for quicker access
        out = self.lin(silu(vec)).chunk(self.multiplier, dim=-1)

        modulation_out1 = ModulationOut(out[0], out[1], out[2])
        modulation_out2 = (
            ModulationOut(out[3], out[4], out[5]) if self.is_double else None
        )

        return modulation_out1, modulation_out2
...
class SingleStreamBlock(nn.Module):
    ...
    def forward(
        self, x: Tensor, vec: Tensor, pe: Tensor, attn_mask=None, modulation_dims=None
    ) -> Tensor:
