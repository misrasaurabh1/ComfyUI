...
import math
from comfy.model_sampling import make_beta_schedule
...
class ModelSamplingDiscrete(torch.nn.Module):
    def __init__(self, model_config=None, zsnr=None):
        ...
        # Load sampling settings from model_config if available
        sampling_settings = model_config.sampling_settings if model_config else {}

        # Extract parameters from sampling settings with default fallbacks
        beta_schedule = sampling_settings.get("beta_schedule", "linear")
        linear_start = sampling_settings.get("linear_start", 0.00085)
        linear_end = sampling_settings.get("linear_end", 0.012)
        timesteps = sampling_settings.get("timesteps", 1000)
        zsnr = zsnr if zsnr is not None else sampling_settings.get("zsnr", False)
    ...
    def sigma(self, timestep):
        t = torch.clamp(
            timestep.to(dtype=torch.float32), min=0, max=self.num_timesteps - 1
        )
        low_idx = t.floor().to(dtype=torch.long)
        high_idx = t.ceil().to(dtype=torch.long)
        w = t.frac()
