...
class ModelSamplingDiscrete(torch.nn.Module):
    ...
    def sigma(self, timestep):
        # Instead of re-calculating the log_sigma values, let's cache the calculated values
        # This optimization assumes that timesteps do not change between different invocations of sigma()

        if not hasattr(self, "_log_sigma_cache"):
            self._log_sigma_cache = {}

        timestep_key = timestep.item()
        if timestep_key in self._log_sigma_cache:
            return self._log_sigma_cache[timestep_key]
        ...
        result = log_sigma.exp().to(timestep.device)

        self._log_sigma_cache[timestep_key] = result
        return result
