...
class ModelSamplingDiscrete(torch.nn.Module):
    def __init__(self, model_config=None, zsnr=None):
        ...
        # Calculate log_sigmas once and store them
        self.log_sigmas = self.sigmas.log()
        self.device = self.log_sigmas.device
    ...
    def timestep(self, sigma):
        # Optimize log computation and device conversion
        log_sigma = sigma.log()
        dists = log_sigma.to(self.device) - self.log_sigmas[:, None]

        abs_dists = dists.abs_()
        argmin_dists = abs_dists.argmin(dim=0)
        reshaped_dists = argmin_dists.view(sigma.shape)

        return reshaped_dists.to(sigma.device)
