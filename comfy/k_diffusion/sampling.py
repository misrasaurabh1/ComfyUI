...
def sample_ipndm_v(model, x, sigmas, extra_args=None, callback=None, disable=None, max_order=4):
    ...
    sin_shape = s_in.shape[0]
    range_max_order = range(max_order - 2)
    ...
    for i in trange(len_sigmas - 1, disable=disable):
        ...
        if order == 1:
            ...
            coeff2 = -(h_n / h_n_1) / 2
            x_next = x_next + h_n * (coeff1 * d_cur + coeff2 * buffer_model[-1])
        elif order == 3:
            h_n = t_next - t_cur
            h_n_1 = t_cur - t_steps[i - 1]
            ...
            x_next = x_next + h_n * (coeff1 * d_cur + coeff2 * buffer_model[-1] + coeff3 * buffer_model[-2])
        elif order == 4:
            h_n = t_next - t_cur
            h_n_1 = t_cur - t_steps[i - 1]
            h_n_2 = t_steps[i - 1] - t_steps[i - 2]
            ...
            x_next = x_next + h_n * (coeff1 * d_cur + coeff2 * buffer_model[-1] + coeff3 * buffer_model[-2] + coeff4 * buffer_model[-3])

        if len(buffer_model) == max_order - 1:
            buffer_model = [buffer_model[k + 1] for k in range_max_order]
