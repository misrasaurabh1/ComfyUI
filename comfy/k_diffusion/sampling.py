...
def sample_dpmpp_2m(model, x, sigmas, extra_args=None, callback=None, disable=None):
    ...
    extra_args = {} if extra_args is None else extra_args
    s_in = x.new_ones([x.shape[0]])
    sigmas_log_neg = -torch.log(sigmas)
    sigma_fns = torch.exp(-sigmas_log_neg)
    ...
    for i in trange(len_sigmas - 1, disable=disable):
        ...
        t, t_next = sigmas_log_neg[i], sigmas_log_neg[i + 1]
        h = t_next - t
        
        if old_denoised is None or sigmas[i + 1] == 0:
            x = (sigma_fns[i + 1] / sigma_fns[i]) * x - (-h).expm1() * denoised
        else:
            h_last = t - sigmas_log_neg[i - 1]
            r = h_last / h
            denoised_d = (1 + 1 / (2 * r)) * denoised - (1 / (2 * r)) * old_denoised
            x = (sigma_fns[i + 1] / sigma_fns[i]) * x - (-h).expm1() * denoised_d
