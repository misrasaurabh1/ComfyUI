...
def sample_deis(model, x, sigmas, extra_args=None, callback=None, disable=None, max_order=3, deis_mode='tab'):
    ...
    s_in = torch.ones([x.size(0)], device=x.device, dtype=x.dtype)
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        order = min(max_order, i + 1) if t_next > 0 else 1

        if order == 1:
            x_next = x_cur + (t_next - t_cur) * d_cur
        else:
            buf_len = len(buffer_model)
            x_next = x_cur + coeff_list[i][0] * d_cur
            for j in range(1, order):
                if j <= buf_len:
                    x_next += coeff_list[i][j] * buffer_model[-j]

        if order > 1:
            if len(buffer_model) == max_order - 1:
                buffer_model = buffer_model[1:]
            buffer_model.append(d_cur.detach())
