...
def sample_ipndm(model, x, sigmas, extra_args=None, callback=None, disable=None, max_order=4):
    ...
    buffer_model = [None] * (max_order - 1)  # initialize buffer with fixed size
    buffer_fill_index = 0  # keep track of buffer fill index
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if order == 1:      # First Euler step.
            ...
            x_next = x_cur + (t_next - t_cur) * (3 * d_cur - buffer_model[0]) / 2
        elif order == 3:    # Use two history points.
            x_next = x_cur + (t_next - t_cur) * (23 * d_cur - 16 * buffer_model[1] + 5 * buffer_model[0]) / 12
        elif order == 4:    # Use three history points.
            x_next = x_cur + (t_next - t_cur) * (55 * d_cur - 59 * buffer_model[2] + 37 * buffer_model[1] - 9 * buffer_model[0]) / 24
