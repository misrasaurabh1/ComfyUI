...
def sample_dpmpp_2s_ancestral_RF(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1., s_noise=1., noise_sampler=None):
    ...
    lambda_fn_cache = {}
    def lambda_fn(sigma):
        if sigma not in lambda_fn_cache:
            lambda_fn_cache[sigma] = ((1 - sigma) / sigma).log()
        return lambda_fn_cache[sigma]

    sigma_fn_cache = {}
    def sigma_fn(lbda):
        if lbda not in sigma_fn_cache:
            sigma_fn_cache[lbda] = (lbda.exp() + 1).reciprocal()
        return sigma_fn_cache[lbda]

    for i in trange(len(sigmas) - 1, disable=disable):
        sig_i = sigmas[i]
        sig_i1 = sigmas[i + 1]

        denoised = model(x, sig_i * s_in, **extra_args)
        downstep_ratio = 1 + (sig_i1 / sig_i - 1) * eta
        sigma_down = sig_i1 * downstep_ratio
        alpha_ip1 = 1 - sig_i1
        alpha_down = 1 - sigma_down
        renoise_coeff = (sig_i1**2 - (sigma_down**2 * alpha_ip1**2) / (alpha_down**2)).sqrt()

        if callback is not None:
            callback({'x': x, 'i': i, 'sigma': sig_i, 'sigma_hat': sig_i, 'denoised': denoised})
        
        if sig_i1 == 0:
            # Euler method
            d = to_d(x, sig_i, denoised)
            x.add_(d, alpha=sigma_down - sig_i)
        else:
            # DPM-Solver++(2S)
            t_i = lambda_fn(sig_i)
            ...
            h = t_down - t_i
            s = t_i + r * h
            sigma_s = 0.9999 if sig_i == 1.0 else sigma_fn(s)
            
            sigma_s_i_ratio = sigma_s / sig_i
            u = torch.add(x, denoised, alpha=-sigma_s_i_ratio).add_(x, alpha=sigma_s_i_ratio)
            D_i = model(u, sigma_s * s_in, **extra_args)

            sigma_down_i_ratio = sigma_down / sig_i
            x.mul_(sigma_down_i_ratio).add_(D_i, alpha=1 - sigma_down_i_ratio)

        # Noise addition
        if sig_i1 > 0 and eta > 0:
            x.mul_(alpha_ip1 / alpha_down).add_(
                noise_sampler(sig_i, sig_i1), 
                alpha=s_noise * renoise_coeff
            )
