...
def get_deis_coeff_list(t_steps, max_order, N=10000, deis_mode='tab'):
    ...
    """
    if deis_mode not in ['tab', 'rhoab']:
        raise ValueError("Invalid deis_mode. Expected 'tab' or 'rhoab'.")

    t_steps, beta_0, beta_1 = deis.edm2t(t_steps)
    C = []
    ...
    if deis_mode == 'tab':
        for i in range(len(t_steps) - 1):
            ...
            if order == 1:
                ...
                taus = torch.linspace(t_cur, t_next, N, device=t_steps.device)
                dtau = (t_next - t_cur) / N
                prev_t = t_steps[i - order + 1:i + 1]

                integrand = deis.cal_intergrand(beta_0, beta_1, taus).to(t_steps.device)
                coeff_temp = [
                    torch.sum(integrand * deis.cal_poly(prev_t, j, taus)) * dtau
                    for j in range(order)
                ]
                C.append(coeff_temp)
    else:
        def get_coefficients(order, prev_t, t_cur, t_next):
            if order == 1:
                return [
                    ((t_next - prev_t[1])**2 - (t_cur - prev_t[1])**2) / (2 * (t_cur - prev_t[1])),
                    (t_next - t_cur)**2 / (2 * (prev_t[1] - t_cur)),
                ]
            elif order == 2:
                return [
                    get_def_intergral_2(prev_t[1], prev_t[2], t_cur, t_next, t_cur),
                    get_def_intergral_2(t_cur, prev_t[2], t_cur, t_next, prev_t[1]),
                    get_def_intergral_2(t_cur, prev_t[1], t_cur, t_next, prev_t[2]),
                ]
            elif order == 3:
                return [
                    get_def_intergral_3(prev_t[1], prev_t[2], prev_t[3], t_cur, t_next, t_cur),
                    get_def_intergral_3(t_cur, prev_t[2], prev_t[3], t_cur, t_next, prev_t[1]),
                    get_def_intergral_3(t_cur, prev_t[1], prev_t[3], t_cur, t_next, prev_t[2]),
                    get_def_intergral_3(t_cur, prev_t[1], prev_t[2], t_cur, t_next, prev_t[3]),
                ]
            return []
        ...
        for i in range(len(t_steps) - 1):
            ...
            if order == 0:
                ...
                prev_t = t_steps[i - order + 1:i + 2]
                C.append(get_coefficients(order, prev_t, t_cur, t_next))
