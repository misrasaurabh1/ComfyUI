...
def load_gligen(sd):
    ...
    sections = ["input_blocks", "middle_block", "output_blocks"]
    for a in sections:
        for b in range(20):
            prefix = f"{a}.{b}."
            keys = [k for k in sd_k if prefix in k and ".fuser." in k]
            
            if keys:
                n_sd = {k.split(".fuser.")[-1]: sd[k] for k in keys}
                query_dim, key_dim = n_sd["linear.weight"].shape
