...
def count_blocks(state_dict_keys, prefix_string):
    """
    Fast block counter by only scanning relevant keys once and using numeric index extraction.
    """
    format_detect = re.search(r'\{[^}]*\}', prefix_string)
    if not format_detect:
        # No {} formatting spec in the prefix, behave like old function (rare path)
        return sum(1 for k in state_dict_keys if k.startswith(prefix_string))
    prefix_split = prefix_string.split('{}')
    prefix_pre = prefix_split[0]
    prefix_post = prefix_split[1] if len(prefix_split) > 1 else ""

    indices = set()
    plen = len(prefix_pre)
    for k in state_dict_keys:
        if k.startswith(prefix_pre) and k.endswith(prefix_post) and len(k) > plen + len(prefix_post):
            maybe_idx = k[plen:len(k)-len(prefix_post) if len(prefix_post)!=0 else None]
            if maybe_idx.isdigit():
                indices.add(int(maybe_idx))
    if not indices:
        return 0
    # Count contiguous [0,1,...,N-1] indices
...
def convert_config(unet_config):
    # Copy and transform the input config as needed.
    ...
    if "attention_resolutions" in new_config:
        ...
        for i in range(len(num_res_blocks)):
            ...
            d = transformer_depth[i] if s in attention_resolutions else 0
            t_in.extend([d] * res)
            t_out.extend([d] * (res + 1))
            s *= 2
...
def unet_config_from_diffusers_unet(state_dict, dtype=None):
    ...
    for i in range(down_blocks):
        ...
        res_blocks = count_blocks(state_dict, f"down_blocks.{i}.resnets." + '{}')
        for ab in range(attn_blocks):
            transformer_count = count_blocks(state_dict, f"down_blocks.{i}.attentions.{ab}.transformer_blocks." + '{}')
            transformer_depth.append(transformer_count)
            if transformer_count > 0:
                match["context_dim"] = state_dict[
                    f"down_blocks.{i}.attentions.{ab}.transformer_blocks.0.attn2.to_k.weight"].shape[1]
    ...
    # All the model configs remain unchanged and inline, for efficiency.
    ...
    supported_models = [LotusD, SDXL, SDXL_refiner, SD21, SD15, SD21_uncliph, SD21_unclipl, SDXL_mid_cnet, SDXL_small_cnet, SDXL_diffusers_inpaint, SSD_1B,
                        Segmind_Vega, KOALA_700M, KOALA_1B, SD09_XS, SD_XS, SDXL_diffusers_ip2p, SD15_diffusers_inpaint]
