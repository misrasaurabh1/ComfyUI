...
def model_lora_keys_clip(model, key_map={}):
    ...
    sdk_set = set(sdk)  # set for O(1) lookup
    LORA_CLIP_MAP_LOCAL = {
        "mlp.fc1": "mlp_fc1",
        "mlp.fc2": "mlp_fc2",
        "self_attn.k_proj": "self_attn_k_proj",
    ...
    # Single pass to add generic weight keys
    suffix = ".weight"
    prefix_t5 = "t5xxl.transformer."
    prefix_hydit = "hydit_clip.transformer.bert."
    len_prefix_t5 = len(prefix_t5)
    len_prefix_hydit = len(prefix_hydit)
    for k in sdk:
        if k.endswith(suffix):
            key_map[f"text_encoders.{k[:-len(suffix)]}"] = k
    ...
    rangestr = range(32)
    for b in rangestr:
        for c, c_mapped in LORA_CLIP_MAP_LOCAL.items():
            # Precompute format strings outside conditional for reuse
            key1 = f"clip_h.transformer.text_model.encoder.layers.{b}.{c}.weight"
            key2 = f"clip_l.transformer.text_model.encoder.layers.{b}.{c}.weight"
            key3 = f"clip_g.transformer.text_model.encoder.layers.{b}.{c}.weight"
            lora_keys = (
                text_model_lora_key.format(b, c_mapped),
                f"lora_te1_text_model_encoder_layers_{b}_{c_mapped}",
                f"text_encoder.text_model.encoder.layers.{b}.{c}",
            )
            # Only fill if present in state dict
            if key1 in sdk_set:
                key_map[lora_keys[0]] = key1
                key_map[lora_keys[1]] = key1
                key_map[lora_keys[2]] = key1

            if key2 in sdk_set:
                key_map[lora_keys[0]] = key2
                key_map[lora_keys[1]] = key2  # SDXL base
                clip_l_present = True
                key_map[lora_keys[2]] = key2

            if key3 in sdk_set:
                clip_g_present = True
                if clip_l_present:
                    lora_key2 = f"lora_te2_text_model_encoder_layers_{b}_{c_mapped}"  # SDXL base
                    key_map[lora_key2] = key3
                    key_map[f"text_encoder_2.text_model.encoder.layers.{b}.{c}"] = key3
                else:
                    lora_key0 = text_model_lora_key.format(b, c_mapped)  # for SDXL-Refiner
                    key_map[lora_key0] = key3
                    key_map[lora_keys[2]] = key3
                    lora_prior = f"lora_prior_te_text_model_encoder_layers_{b}_{c_mapped}"  # cascade lora
                    key_map[lora_prior] = key3

    # Fast sequential scan for special prefix cases, do not nest logic deep
    t5xxl_prefix = prefix_t5
    hydit_prefix = prefix_hydit
    for k in sdk:
        if k.endswith(suffix):
            if k.startswith(t5xxl_prefix): # OneTrainer SD3 and Flux lora
                l_key = k[len_prefix_t5:-len(suffix)]
                t5_index = 1
                if clip_g_present:
                    t5_index += 1
                if clip_l_present:
                    t5_index += 1
                    if t5_index == 2:
                        key_map[f"lora_te{t5_index}_{l_key.replace('.', '_')}"] = k  # OneTrainer Flux
                        t5_index += 1
                key_map[f"lora_te{t5_index}_{l_key.replace('.', '_')}"] = k
            elif k.startswith(hydit_prefix): # HunyuanDiT Lora
                l_key = k[len_prefix_hydit:-len(suffix)]
                lora_key = f"lora_te1_{l_key.replace('.', '_')}"
                key_map[lora_key] = k

    # Final single checks for text_projection
    key_cg_proj = "clip_g.transformer.text_projection.weight"
    key_cl_proj = "clip_l.transformer.text_projection.weight"
    if key_cg_proj in sdk_set:
        key_map["lora_prior_te_text_projection"] = key_cg_proj  # cascade lora?
        # key_map["text_encoder.text_projection"] = key_cg_proj #TODO: check if other lora have the text_projection too
        key_map["lora_te2_text_projection"] = key_cg_proj  # OneTrainer SD3 lora
    if key_cl_proj in sdk_set:
        key_map["lora_te1_text_projection"] = key_cl_proj  # OneTrainer SD3 lora, not necessary but omits warning
