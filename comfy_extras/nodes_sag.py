...
def gaussian_blur_2d(img, kernel_size, sigma):
    ...
    ksize_half = (kernel_size - 1) // 2

    # Use arange for kernel coordinates calculation
    x = torch.arange(-ksize_half, ksize_half + 1, device=img.device, dtype=img.dtype)
    ...
    # Make 2D kernel
    kernel2d = x_kernel[:, None] @ x_kernel[None, :]
    c = img.shape[-3]

    # Repeat kernel for each channel
    kernel2d = kernel2d[None, None, :, :].repeat(c, 1, 1, 1)

    # Use only two values for symmetric padding: (pad_left/right, pad_top/bottom)
    padding = ksize_half

    # Apply reflection padding
    img = F.pad(img, (padding, padding, padding, padding), mode="reflect")
    img = F.conv2d(img, kernel2d, groups=c)
    return img
